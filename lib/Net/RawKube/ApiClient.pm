...
sub _cfg {'Net::RawKube::Configuration'}
...
sub call_api {
    ...
    $self->{ua}->timeout($self->{http_timeout} || $Net::RawKube::Configuration::http_timeout); 
    $self->{ua}->agent($self->{http_user_agent} || $Net::RawKube::Configuration::http_user_agent);
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("Net::RawKube::Object::$class")->new;
        if (ref $data eq "HASH") {
...
sub get_api_key_with_prefix
{
	...
	my $api_key = $Net::RawKube::Configuration::api_key->{$key_name};
	
	return unless $api_key;
	
	my $prefix = $Net::RawKube::Configuration::api_key_prefix->{$key_name};
