...
#
package Net::RawKube::ApiFactory;
...
usesub Net::RawKube::Object;

use Net::RawKube::ApiClient;

=head1 Name

	Net::RawKube::ApiFactory - constructs APIs to retrieve Net::RawKube objects
...
=head1 Synopsis
	...
	my $api_factory = Net::RawKube::ApiFactory->new( ... ); # any args for ApiClient constructor
							  
	# later...
	my $pet_api = $api_factory->get_api('Pet');  
	
	# $pet_api isa Net::RawKube::PetApi
...
my %_apis = map { $_ =~ /^Net::RawKube::(.*)$/; $1 => $_ } 
			grep {$_ =~ /Api$/} 
			usesub 'Net::RawKube';
...
sub new {
    my ($class, %p) = (shift, @_);
	$p{api_client} = Net::RawKube::ApiClient->instance(%p);			
