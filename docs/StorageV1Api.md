# Net::RawKube::StorageV1Api

## Load the API package
```perl
use Net::RawKube::Object::StorageV1Api;
```
...
> V1StorageClass create_storage_class(body => $body, pretty => $pretty)
...
my $api_instance = Net::RawKube::StorageV1Api->new();
my $body = Net::RawKube::Object::V1StorageClass->new(); # V1StorageClass | 
...
if ($@) {
    warn "Exception when calling StorageV1Api->create_storage_class: $@\n";
}
```
...
if ($@) {
    warn "Exception when calling StorageV1Api->delete_collection_storage_class: $@\n";
}
```
...
if ($@) {
    warn "Exception when calling StorageV1Api->delete_storage_class: $@\n";
}
```
...
if ($@) {
    warn "Exception when calling StorageV1Api->get_api_resources: $@\n";
}
```
...
# **list_storage_class**
> V1StorageClassList list_storage_class(pretty => $pretty, field_selector => $field_selector, include_uninitialized => $include_uninitialized, label_selector => $label_selector, resource_version => $resource_version, timeout_seconds => $timeout_seconds, watch => $watch)
...
if ($@) {
    warn "Exception when calling StorageV1Api->list_storage_class: $@\n";
}
```
...
# **patch_storage_class**
> V1StorageClass patch_storage_class(name => $name, body => $body, pretty => $pretty)
...
if ($@) {
    warn "Exception when calling StorageV1Api->patch_storage_class: $@\n";
}
```
...
# **read_storage_class**
> V1StorageClass read_storage_class(name => $name, pretty => $pretty, exact => $exact, export => $export)
...
if ($@) {
    warn "Exception when calling StorageV1Api->read_storage_class: $@\n";
}
```
...
> V1StorageClass replace_storage_class(name => $name, body => $body, pretty => $pretty)
...
if ($@) {
    warn "Exception when calling StorageV1Api->replace_storage_class: $@\n";
}
```
